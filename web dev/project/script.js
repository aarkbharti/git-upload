...
async function searchWeather(cityName) {
  try {
    ...
    const responsePromise = fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${encoded}&appid=${API_KEY}`
    );
    ...
    // API may return cod as number or string; normalize to string
    if (String(weatherData.cod) === "200") {
      ...
      container.innerHTML = `
        <h3>Weather Info</h3>
        <p>${weatherData.message || 'Unable to fetch weather.'}</p>
      `;
      logMessage(`Error: ${weatherData.message || 'Unknown error'}`);
    }
  } catch (e) {
    console.error(e);
    container.innerHTML = `
      <h3>Weather Info</h3>
      <p>Network or server error. Please try again.</p>
    `;
    logMessage(`Exception: ${e.message || e}`);
  }
}
